@testset "Optimal Transport" begin
    ...
    @testset "wasserstein" begin
        ...
        @test wasserstein(Normal(0, 1), Normal(3, 1); p = 1) ≈ 3.0 rtol = 1.0e-3
        @test wasserstein(Normal(0, 1), Normal(3, 1); p = 2) ≈ 3.0 rtol = 1.0e-3
        # W_2 between Gaussians with equal mean is |σ₁ - σ₂|
        @test wasserstein(Normal(0, 1), Normal(0, 3); p = 2) ≈ 2.0 rtol = 1.0e-3
    ...
    @testset "transportmap / pushforward" begin
        ...
        @test T(0.0) ≈ 3.0 rtol = 1.0e-6
        @test T(1.0) ≈ 4.0 rtol = 1.0e-6

        # scale map: Normal(0,1) → Normal(0,2) doubles each quantile
        T2 = transportmap(Normal(0, 1), Normal(0, 2))
        @test T2(1.0) ≈ 2.0 rtol = 1.0e-6
        @test T2(-1.5) ≈ -3.0 rtol = 1.0e-6
        ...
        @test mean(stressed) ≈ mean(s) + 3 rtol = 1.0e-6
        @test stressed ≈ T.(s)
    ...
    @testset "robustvalue (Wasserstein DRO)" begin
        ...
        for (α, p, r) in ((0.95, 2, 250.0), (0.995, 2, 250.0), (0.9, 1, 100.0))
            bound = CTE(α)(s) + r * (1 - α)^(-1 / p)
            @test robustvalue(CTE(α), s; radius = r, p = p) ≈ bound rtol = 1.0e-8
        end
        ...
        let sm = collect(1.0:10.0)
            for (α, p, r) in ((0.25, 1, 3.0), (0.7, 2, 2.0), (0.0, 1, 1.5))
                @test robustvalue(CTE(α), sm; radius = r, p = p) ≈
                    CTE(α)(sm) + r * (1 - α)^(-1 / p) rtol = 1.0e-12
            end
        end
        ...
        let st = [fill(0.0, 80); fill(50.0, 15); fill(100.0, 5)]   # heavy tie at 50
            @test robustvalue(CTE(0.9), st; radius = 4.0, p = 2) ≈
                CTE(0.9)(st) + 4.0 * (1 - 0.9)^(-1 / 2) rtol = 1.0e-12
        end
        ...
        let sm = collect(1.0:10.0)
            ...
            @test wasserstein(sc, sm; p = 2) ≈ 1.0 rtol = 1.0e-12
